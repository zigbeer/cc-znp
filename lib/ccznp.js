...
CcZnp.prototype.init = function (spCfg, callback) {
    ...
    var parseMtIncomingData = this._parseMtIncomingData,
...
/*********************************/
// exmaple: ccznp.sysRequest(), ccznp.zdoRequest()
...
CcZnp.prototype._sendSREQ = function (argObj, callback) {
    ...
    this._unpi.send('SREQ', argObj.subsys, argObj.cmd, payload);
};
...
CcZnp.prototype._parseMtIncomingData = function (data) {
    ...
    try {
        ...
        data.type = argObj.type;        // make sure data.type will be string
...
CcZnp.prototype._mtIncomingDataHdlr = function (err, data) {
    ...
    if (data.type === 'SRSP') {
        ...
        if ((subsys === 'SYS' && cmd === 'resetReq') || (subsys === 'SAPI' && cmd === 'systemReset'))
            rxEvt = 'AREQ:SYS:RESET';
    }

    if (rxEvt)
        ccznp.emit(rxEvt, data);
};
